...
#Claire McLeod, cm.mcleod@gmail.com, last updated 2015-05-05
#
...
blindPath=basePath + '\\blindedImages\\'
origPath=basePath + '\\originalImages\\'
blindingNotesPath=basePath + '\\blindingNotes\\'
nameList=[]
...
if os.path.exists(blindingNotesPath):
    ...
    numAssignments=np.loadtxt(blindingNotesPath+'assignList.txt')
    nameList=np.ndarray.tolist(np.genfromtxt(blindingNotesPath+'listNames.csv', delimiter=None, dtype=None))
...
if not os.path.exists(blindingNotesPath):
    ...
    np.savetxt(blindingNotesPath+'assignList.txt', numAssignments)
    assignIndex=0
...
with open(blindingNotesPath+'matchedFilenames.csv', 'wb') as csvfile:
    listwriter=csv.writer(csvfile, delimiter=',')
    for item in filesMatched:
        listwriter.writerow(item)

with open(blindingNotesPath+'listNames.csv', 'wb') as csvfile:
    listwriter=csv.writer(csvfile, delimiter=',')
